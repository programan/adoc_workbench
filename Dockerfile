...
RUN set -x \
  ...
  && echo -e '#! /bin/bash\nset -eu\n' > adocpdf.sh \
  && echo -en "asciidoctor-pdf -r asciidoctor-diagram -r asciidoctor-mathematical -a mathematical-format=svg -a pdf-stylesdir=${PDF_THEME_DIR} -a pdf-style=default-with-fallback-font-theme.yml -a scripts=cjk" >> adocpdf.sh \
  && echo ' "$@"' >> adocpdf.sh \
